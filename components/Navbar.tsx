...
const FacebookLogo = memo(({ className }: { className?: string }) => (
    ...
    <>
      <nav className="relative z-50 w-full max-w-[1600px] mx-auto px-4 py-3 md:px-6 md:py-6 flex justify-center items-center pointer-events-none select-none shrink-0">
        <div className="flex items-center gap-2 md:gap-4 pointer-events-auto">
          <div className="flex items-center p-1.5 pl-3 md:pl-4 gap-3 bg-white/10 backdrop-blur-[32px] border border-white/10 shadow-[0_24px_48px_-12px_rgba(0,0,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.1)] rounded-full transition-all duration-500 hover:bg-white/15 transform-gpu ring-1 ring-white/[0.05]">
            <div
              className="pr-2 md:pr-3 flex items-center gap-2.5 group cursor-pointer border-r border-white/10"
              onClick={handleHomeClick}
            >
              <span className="font-designer text-xl md:text-2xl tracking-tighter text-white hidden sm:block opacity-90 group-hover:opacity-100 transition-opacity">
                Abdou
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <motion.span
                        layoutId="nav-pill"
                        className="absolute inset-0 bg-gradient-to-b from-cyan-500/20 to-cyan-600/10 backdrop-blur-xl rounded-full z-0 border border-cyan-400/30 shadow-[0_2px_14px_-3px_rgba(34,211,238,0.4),inset_0_1px_2px_rgba(255,255,255,0.1)]"
                        transition={{
                          type: "spring",
                          bounce: 0.28,
                          duration: 0.6,
                    ...
                    <item.icon
                      ...
                      className={`relative z-10 transition-transform duration-300 ${isActive ? "text-cyan-400 scale-110" : "opacity-70 text-gray-400 group-hover/item:scale-105 group-hover/item:text-white"}`}
                    />
                    <span
                      className={`relative z-10 tracking-tight transition-colors duration-300 ${isActive ? "font-bold text-cyan-400" : "text-gray-400 group-hover/item:text-white"}`}
                    >
                      {item.name}
                    </span>
                  </button>
            ...
            <button
              className="md:hidden w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors border border-white/10 text-white"
            ...
            <nav className="flex flex-col gap-8 text-center">
              {menuItems.map((item, index) => (
                <motion.button
                  ...
                  className={`text-3xl font-display font-bold flex items-center justify-center gap-4 ${
